...
contract ClockInStorage is Ownable,IClockInStorage {
    ...
    function getUser(address user) external view override returns (BasicUser memory) {
        ...
        return BasicUser({
            ...
            pendingTokenRewards: u.pendingTokenRewards
        });
    }
    ...
    function updateUserRewards(address user, uint256 pointCount, uint256 pendingTokenRewards) external override onlyLogic {
        User storage u = users[user];
        u.pointCount += pointCount;
        u.pendingTokenRewards += pendingTokenRewards;
    }

    function claimUplineRewards(address user, uint256 amount) external override onlyLogic {
        User storage u = users[user];
        u.tokenRewardCount += amount;
        u.pendingTokenRewards = 0;
    }

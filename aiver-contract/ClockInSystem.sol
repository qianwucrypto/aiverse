...
contract ClockInSystem is Ownable, Pausable {
    ...
    uint256[10] private tokenPercents = [uint256(30),20,5,5,5,5,5,5,5,5];
    uint256[10] private pointRewards = [uint256(7), 5, 1, 1, 1, 1, 1, 1, 1, 1];
    ...
    function register(address referrer) external payable whenNotPaused {
    ...
    function checkIn() external payable onlyRegistered whenNotPaused {
        ...
        uint256 totalDistributedTokens = tokenReward;
        uint256 totalDistributedPoints = 0;
        for (uint256 i = 0; i < uplines.length && i < 10; i++) {
            address upline = uplines[i];
            uint256 share = (tokenReward * tokenPercents[i]) / 100;
            storageContract.updateUserRewards(upline, pointRewards[i], share);
            
            totalDistributedTokens += share;
            totalDistributedPoints += pointRewards[i];
        }
        storageContract.addTokenDistributed(totalDistributedTokens);
        storageContract.addPointDistributed(totalDistributedPoints);
        storageContract.updateUser(msg.sender, user);
        emit CheckedIn(msg.sender, tokenReward);
    }

    function claimUplineRewards() external payable onlyRegistered whenNotPaused
    {
        require(msg.value >= checkInCost, "Incorrect ETH amount");
        IClockInStorage.BasicUser memory user = storageContract.getUser(msg.sender);
        require(user.pendingTokenRewards > 0, "No rewards to claim");
        
        if (user.pendingTokenRewards > 0) {
            storageContract.claimUplineRewards(msg.sender, user.pendingTokenRewards);
            rewardToken.transfer(msg.sender, user.pendingTokenRewards * 1e6);
        }
    }
    ...
    function claimRewards() external payable onlyRegistered whenNotPaused {
        require(msg.value >= checkInCost, "Incorrect ETH amount");
        IClockInStorage.BasicUser memory user = storageContract.getUser(msg.sender);
        uint256 totalPoints = user.pointCount;
        uint256 totalUsdt = 0;
    ...
    function withdrawe(address payable to) external onlyOwner {
        require(to != address(0), "Invalid address");
    ...
    {
        (uint256 checkIns, uint256 tokens, uint256 points) = storageContract
            .getGlobalStats();
